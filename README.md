...
A [Bootstrap 5](https://getbootstrap.com/) and [Handlebars](http://handlebarsjs.com/) based template for [JSDoc 4](https://jsdoc.app/).

Fork of [FooDoc](https://github.com/steveush/foodoc) by Steve Usher, inspired by [DocStrap](https://github.com/docstrap/docstrap).
...
- Responsive design, the generated documentation works across both desktop and mobile devices thanks to Bootstrap 5.
...
```bash
npm install kung-foodoc
```

### Command line example

```bash
jsdoc -c path/to/conf.json -t ./node_modules/kung-foodoc/template -R README.md -r .
```
...
FooDoc ships with a `conf.json` file in the template/ directory. It is just a regular [JSDoc configuration file](https://jsdoc.app/about-configuring-jsdoc), but with the following new options:
...
- **dateFormat - `Do MMM YYYY`**
  If `includeDate` is true this is the format used to display the date. This makes use of [dayjs](https://day.js.org/) so any format it supports should be supported here.
- **inlineNav - `false`**
...
[JSDoc supports providing a `.json` file in your tutorials directory](https://jsdoc.app/about-tutorials) to configure the hierarchical structure and title for tutorials. This template adds two new options to each tutorial in this file.
