		...
		return this.db.search(query).then(function(results){
			...
			$.each(pages, function(i, page){
				...
				$.each(page, function(i, result){
					...
					var $li = $('<li/>'),
						$a = $('<a/>', {href: result.id, 'class': 'lunr-search-result-title'}).html(result.title).prepend($('<span/>', {'class':'lunr-search-result-kind',text:result.kind + ': '}));
					$li.append($a);
					$li.append($('<p/>', {'class': 'lunr-search-result-url'}).text($a.prop('href')));
					$li.append($('<p/>', {'class': 'lunr-search-result-summary'}).text(result.summary));
					$ul.append($li);
				});
				self.$body.append($ul);
			});
